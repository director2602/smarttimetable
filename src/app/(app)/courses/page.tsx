...
export default async function CoursesPage() {
  ...
  const canDelete = resolvePermission(user.role as Role, overrides, "COURSE_DELETE");
          ...
          <tbody>{rows.map((r) => (
            <Fragment key={r.id}>
              <tr className="border-t border-slate-100">
                ...
                <td className="px-4 py-2 text-right"><CourseEditForm course={r} canEdit={canEdit} canDelete={canDelete} /></td>
              </tr>
            </Fragment>
          ))}</tbody>
        </table>
